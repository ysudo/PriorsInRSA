...
eta <- function(u,prob) {
	if (u=='all') {
		return(prob('a')*surprisal(prob('a')))
	} else if (u=='some') {
	return((prob('a') * surprisal(prob('a'))) + (prob('sbna') * surprisal(prob('sbna'))))
	}
}

S <- function(n, u, w, prob) {
	return(exp(lambda * (log(L(n-1, w, u, prob)) + (k*eta(u, prob))))/(exp(lambda * (log(L(n-1, w, 'all', prob)) + (k*eta('all', prob)))) + exp(lambda * (log(L(n-1, w, 'some', prob)) + (k*eta('some', prob))))))
}
...
names(L1to5df) <- as.numeric(p_grid)
